...
pub async fn fetch_players(
    ...
    if debug {
        ...
        eprintln!("HEADERS:");
        for (k, v) in req.headers().iter() { eprintln!("  {}: {:?}", k, v); }
    }

    let v = builder.send().await?.error_for_status()?.json::<Value>().await?;
    Ok(v)
}
...
pub fn build_player_weeks_points(
    ...
    for p in arr {
        ...
        let name = p.get("fullName").and_then(|v| v.as_str()).unwrap_or("Unknown").to_string();

        let mut wk = Vec::new();
        if let Some(stats) = p.get("stats").and_then(|v| v.as_array()) {
            for s in stats {
                let sid = s.get("seasonId").and_then(|v| v.as_u64());
                let week = s.get("scoringPeriodId").and_then(|v| v.as_u64()).map(|x| x as u16);
                let src = s.get("statSourceId").and_then(|v| v.as_u64());
                if sid == Some(season as u64) && src == Some(0) {
                    if let Some(w) = week {
                        if weeks.contains(&w) {
                            let points = s.get("appliedTotal").and_then(|v| v.as_f64()).unwrap_or(0.0);
        ...
        out.push(PlayerWeekPoints { id, name, weeks: wk });
    }
