...
mod database_tests {
    ...
    #[test]
    fn test_get_projection_analysis_specific_week() {
        let mut db = create_test_db();
        ...
        // Insert stats for multiple weeks with different errors
        let stats_week1 = PlayerWeeklyStats {
            player_id: PlayerId::new(12345),
            season: Season::new(2023),
            week: Week::new(1),
            projected_points: Some(20.0),
            actual_points: Some(15.0), // 5.0 error
            created_at: 0,
            updated_at: 0,
        };
        db.upsert_weekly_stats(&stats_week1, false).unwrap();

        let stats_week2 = PlayerWeeklyStats {
            player_id: PlayerId::new(12345),
            season: Season::new(2023),
            week: Week::new(2),
            projected_points: Some(15.0),
            actual_points: Some(20.0), // -5.0 error
            created_at: 0,
            updated_at: 0,
        };
        db.upsert_weekly_stats(&stats_week2, false).unwrap();

        // Test analysis for specific week - analyze data before week 2 (so includes week 1)
        let analysis = db
            .get_projection_analysis(Season::new(2023), Some(Week::new(2)), Some(10))
            .unwrap();

        assert_eq!(analysis.len(), 1);
        let player_analysis = &analysis[0];
        assert_eq!(player_analysis.games_count, 1);
        assert!((player_analysis.avg_error - 5.0).abs() < 0.01);
    }
    ...
    fn test_estimate_week_performance_no_data() {
        ...
        let estimates = db
            .estimate_week_performance(Season::new(2023), Week::new(5), &projected_data, Some(10))
            .unwrap();
    ...
    fn test_estimate_week_performance_limited_data() {
        ...
        // Insert only 2 weeks of data (less than 3 minimum)
        for week in 1..=2 {
            let stats = PlayerWeeklyStats {
                player_id: PlayerId::new(12345),
                season: Season::new(2023),
        ...
        let projected_data = vec![(PlayerId::new(12345), 20.0)];

        let estimates = db
            .estimate_week_performance(Season::new(2023), Week::new(3), &projected_data, Some(10))
            .unwrap();

        assert_eq!(estimates.len(), 1);
        let estimate = &estimates[0];

        // With limited data, should not make large adjustments
        assert!((estimate.estimated_points - 20.0).abs() < 1.0);
        assert!(estimate.confidence < 0.5);
        assert!(estimate.reasoning.contains("Limited data"));
    }
    ...
    fn test_database_schema_initialization() {
        ...
        assert!(table_names.contains(&"player_weekly_stats".to_string()));
    }
    ...
    fn test_database_indexes_exist() {
        ...
        assert!(index_names
            .iter()
            .any(|name| name.contains("idx_player_season_week")));
        assert!(index_names
            .iter()
            .any(|name| name.contains("idx_projection_diff")));
    }
    ...
    fn test_clear_all_data() {
        ...
        // Verify we can still add data after clearing (tables still exist)
        let player = Player {
            player_id: PlayerId::new(12345),
            name: "Test Player".to_string(),
            position: "QB".to_string(),
